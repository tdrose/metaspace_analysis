---
title: "ORA analysis for METASPACE analysis results"
...
# Enrichment
```{r}
query_sf = read.csv(file.path(enrichment_path, "pos_lip_Kidney0.csv"))[,2] %>% unlist() %>% unique()

cat( paste0(sum(query_sf %in% bg_sf), ' of ', length(query_sf), ' query molecules are in the universe.'))
```
...
### LION
```{r}
result_simple = bmetenrichr:::Run_bootstrap_ORA(marker_list=query_sf, 
...
Results
```{r}
tibble(result_simple[[1]]$clean_enrich_res) %>%
  left_join(bmetenrichr:::LION_LUT, by=c('term'='ID')) %>%
  filter(q.value_median<=0.1) %>%
  select(name, q.value_median, everything())
```
...
result_simple = bmetenrichr:::Run_bootstrap_ORA(marker_list=query_sf, 
                                                ...
                                                mass_range_ppm = 3,
                                                only_HMDB = T, annot_weights = NULL,
                                                n_bootstraps = 2, sample_core_metab_only = F,
                                                boot_fract_cutoff = 0.5, q.val_cutoff = 0.2,
                                                selected_terms = NULL)
```
...
result_simple = bmetenrichr:::Run_bootstrap_ORA(marker_list=query_sf, 
                                                ...
                                                only_HMDB = T, annot_weights = NULL,
                                                n_bootstraps = 2, sample_core_metab_only = F,
                                                boot_fract_cutoff = 0.5, q.val_cutoff = 0.3,
                                                selected_terms = NULL)
```
...
# Results
```{r}
tibble(enrichment_results[["pos_lip_Brain0.csv"]]$clean_enrich_res) %>%
  left_join(bmetenrichr:::LION_LUT, by=c('term'='ID')) %>%
  filter(q.value_median<=0.1) %>%
  select(name, q.value_median, everything()) %>% dim()
```


```{r}
for (filename in names(enrichment_results)){
  tibble(enrichment_results[[filename]]$clean_enrich_res) %>%
  left_join(bmetenrichr:::LION_LUT, by=c('term'='ID')) %>%
  filter(q.value_median<=0.1) %>%
  select(name, q.value_median, everything()) %>% dim() %>% print()
}
```
