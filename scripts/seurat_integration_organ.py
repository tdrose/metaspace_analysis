...
def process_data(adata, file, path, batch_key='ds', reduction='rpca'):
    ...
    r('features <- rownames(seurat)')
    r('batch_list <- lapply(X = batch_list, FUN = function(x) {x <- RunPCA(ScaleData(x, features = features, verbose = FALSE), features = features, verbose = FALSE)})')
    r(f'saveRDS(batch_list, file = "/g/alexandr/tim/brain_batch_list.rds")')
    r(f'anchors <- FindIntegrationAnchors(batch_list, anchor.features = features, reduction="{reduction}", k.filter = 50, scale=FALSE)')
