...
```{r}
#library(devtools)
#install_github("Bisho2122/bmetenrichr")
#install_github("cran/hypergea")
```
...
enrich = function(y, universe=NULL, metabo_classes_univ, metabo_subclasses_univ, metabo_pathway_univ, boot_fract_cutoff=0.5, min_annot = 3, q.val_cutoff = 0.2){
    #tissue_final = simplify_hypergeom_bootstrap(bootstrap_list = y,
    #                                 term_list = metabo_tissue_loc, 
    #                                 universe = universe, core_metabo_only = F,
    ...
    class_final = simplify_hypergeom_bootstrap(bootstrap_list = y,
                                                  ...
                                                  universe = universe, 
                                                  core_metabo_only = F,
                                                  boot_fract_cutoff = boot_fract_cutoff, 
                                                  min_annot = min_annot, 
                                                  q.val_cutoff = q.val_cutoff)
    ...
    return(list('class'=class_final,
                'subclass'=subclass_final,
                'pathway'=pathway_final)
           )
}
...
```{r}
# File locations - Copied from config.py

enrichment_dir = '/g/alexandr/tim/metaspace_evaluation/230201/enrichment/'
...
bg_sf <- read_csv(paste0(enrichment_dir, 'bg_', polarity, '.csv'))[2] %>% unlist() %>% unique()
bg_name = metaspace_databases$name[which(metaspace_databases$formula %in% 
                                         bg_sf & metaspace_databases$db == "HMDB")] %>% unique()

for(i in files){
    if(str_starts(i, paste0('features_', polarity))){
        
        cat('###\n')
        print(paste0('Starting analysis for: ', i)) 
        
        # Prepare file
        query_sf <- read_csv(paste0(enrichment_dir, i))[2] %>% unlist() %>% unique()
        x = get_metabo_iso(sf_vec = query_sf, consider_isobars = F)
        y = metabo_bootstrap(annot_list = x)
        
        # Enrichment w/ BG
        enr_wbg <- enrich(y, 
                          universe=bg_name, 
                          metabo_classes_univ=metabo_classes_univ, 
                          metabo_subclasses_univ=metabo_subclasses_univ, 
                          metabo_pathway_univ=metabo_pathway_univ,
                          boot_fract_cutoff=0.3, 
                          min_annot = 1, 
                          q.val_cutoff = 0.2
                          )
        
        saveRDS(enr_wbg, paste0(enrichment_dir, 
                                 'enrichment_', polarity, '_wBG_', 
                                 str_split(i, pattern='\\.')[[1]][1],
                                 '.rds'
                                ))
        print('Finished enrichment w/ background')
        ...
        saveRDS(enr_wobg, paste0(enrichment_dir, 
                                 'enrichment_', polarity, '_woBG_', 
                                 str_split(i, pattern='\\.')[[1]][1],
                                 '.rds'
                                ))
        
        #print('Finished enrichment w/o background')
        cat('\n\n')
    }
